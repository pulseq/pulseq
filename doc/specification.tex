...
The specification described in Section~\ref{sec:specification} can be implemented as a binary file. The same general principles apply with specific modifications outlined here. In general, \textbf{integer} types are stored as 64-bit unsigned integers while \textbf{floating-point} numbers are in the 64-bit IEEE 754 floating-point format.

The basic structure of a binary \emph{pulseq} file is depicted below,
\texttt{
\begin{center}
...
A binary \textit{Pulseq} file begins with the 64 bit code \verb.0x0170756C73657102. (the characters \verb.pulseq. enclosed by \verb.0x01. and \verb.0x02.)  followed by an integer describing the file version (currently always version 1). The remaining file is made up of multiple sections each with an integer section code followed by section-specific storage. The section codes corresponding to text file tags are
...
The remaining sections are declared exactly as in Section~\ref{sec:specification} with storage defined by the value type. For \verb.integer. types an integer is used and for \verb.float. types a floating point number is used.
