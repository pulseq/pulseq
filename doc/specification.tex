...
1.5.1 & 14 Aug 2025 & \makecell{Minor format revision, new extensions and bugfixes: \\
New concept of \textit{required extensions} wich must be supported by the interpeter; \\
Introduce new Rotation Extension; \\
Introduce new RF Shimming Extension. \\
} \\ 
1.5.2 & 28 Apr 2026 & \makecell{Minor format update for the binary format and bugfixes. \\
...
\lstinputlisting{../tests/legacy/fiddisp.seq}
...
Extensions concept allows for implementing additional pulse sequence format features without requiring major revisions of the Pulseq format specification. Extensions can be defined in the design tool in a good hope that the particular interpreter will know how to handle it. The interpreter MUST detect unknown extensions and MAY chose to ignore them. It is recommended that the interpreter issues a warning in this case. Since the format revision v1.5.1 there is a concept of \textit{required extensions}, that is the extensions that are critical for the interpretability of the particular pulse sequence. All extensions listed under the definition \verb.RequiredExtensions. must be supported by the interpreter. If any of the declared required extensions are unknown to the interpreter, it MUST generate an error and MUST not attempt executing the sequence. The definition \verb.RequiredExtensions. contains a space-separated list  of extension's \verb.STRING_ID(s)..
...
\textbf{Rotation Extension}

The \textit{rotation extension} has been introduced in revision 1.5.1. It allows the pulse sequence developer to reuse the gradient object, expecially in case of arbitrary shapes, e.g. as needed for multi-shot spiral seuqnces, and specify a rotation operation that is applied to the entire block as a whole. Only one \textit{rotation extension} object can be specified per block. Each rotation operation is described as a normalized rotation quaternion as follows:
...
Rotation quaternions are represented by their real part \verb.RotQuat0., which corresponds to the cosine of the halved rotation angle, and the three imaginary components \verb.RotQuatX., \verb.RotQuatY., and \verb.RotQuatZ., which together describe the axis of rotation, scaled by the sine of the halved rotation angle. In the example above, line 1 defines a rotation by 14.77\textdegree\, about the negative Z axis, whereas line 2 specifies a unity transform (zero rotation). Please consult functions \lstinline{mr.aux.quat.toRotMat()} and \lstinline{mr.aux.quat.fromRotMat()} for more information on transformations between the rotation matrices and quaternions. A great care should be taken when implementing rotation support in interpreters because of the misinterpretation possibilities (e.g. due to inconsistencies between raw-major vs. column-major matrix math libraries). Therefore we recommend to compare measurement results generated by the sequences using the \textit{rotation extension} with the sequence relying on explicit rotations using \lstinline{mr.rotate()} or \lstinline{mr.rotate3D()} functions.

\textbf{RF Shimming Extension}

The \textit{RF shimming} extension has been introduced in revision 1.5.1. Only one \textit{RF shimming} extension object can be specified per block. RF shimming parameters only act on the RF pulse in the current block. This means RF shimming parameters specified for a block containing no RF have no effect and can be ignored. RF shimming settings are highly experimental and more experience is needed to possibly allow future revisions of the extension. For instance RF shimming parameters start with explicitly specifying for how many RF transmit channels these are designed. If this number does not correspond to the number of channels available in the hardware, the interpreter should probably ignore this setting and may generate an error or warning.
...
In the example above, line 1 defines a 2to-channel RF shimming with both channels sharing the nominal amplitude but a phase shift of 90\textdegree\, is applied to the second channel. Line 2 applies a phase shift of 180\textdegree\, to the second channel, whereas in line 3 all phases are at 0 but the amplitude of channel 1 is scaled down by a factor of $0.7$.
...
The specification described in Section~\ref{sec:specification} can also be implemented as a binary file. The binary stream starts with a fixed 8-byte file header followed by version numbers and then a list of sections.
...
\hline
\multicolumn{8}{|c|}{version major (int64)} \\
\hline
\multicolumn{8}{|c|}{version minor (int64)} \\
\hline
\multicolumn{8}{|c|}{version revision (int64)} \\
\hline
\multicolumn{8}{|c|}{section code (int64)} \\
\hline
\multicolumn{8}{|c|}{section-specific payload} \\
\hline
\multicolumn{8}{|c|}{section code (int64)} \\
\hline
\multicolumn{8}{|c|}{section-specific payload} \\
...
Unless specified otherwise, integer values are little-endian and stored as \verb.int64. or \verb.int32., and floating-point values are \verb.float64..

\subsection{Binary numeric conventions}

The current binary implementation uses a unified representation for time-like quantities:
\begin{itemize}
\item Generic time values are stored as \verb.int64. picoseconds (ps). This applies to RF center/delay, arbitrary-gradient delay, trapezoid rise/flat/fall/delay, ADC dwell/delay, trigger delay/duration, and soft-delay offset.
\item Block duration is a special case and is stored as \verb.int64. in units of \verb.BlockDurationRaster. (not in ps).
\item Shape payload samples in binary files are currently written as \verb.float32. compressed values.
\end{itemize}
...
\verb.[DELAYS]. (legacy, deprecated) & \verb.0xFFFFFFFF 0x00000007. \\
\verb.[SHAPES]. & \verb.0xFFFFFFFF 0x00000008. \\
\verb.[EXTENSIONS]. & \verb.0xFFFFFFFF 0x00000009. \\
\verb.extension TRIGGERS. payload & \verb.0xFFFFFFFF 0x0000000A. \\
\verb.extension LABELSET. payload & \verb.0xFFFFFFFF 0x0000000B. \\
\verb.extension LABELINC. payload & \verb.0xFFFFFFFF 0x0000000C. \\
\verb.extension DELAYS. payload (soft delay) & \verb.0xFFFFFFFF 0x0000000D. \\
\verb.extension RF\_SHIMS. payload & \verb.0xFFFFFFFF 0x0000000E. \\
\verb.extension ROTATIONS. payload & \verb.0xFFFFFFFF 0x0000000F. \\
\verb.[SIGNATURE]. binary payload & \verb.0xFFFFFFFF 0x00FFFFFF. \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Section payload formats}

Each section starts with its \verb.section code (int64)., see the table above. The payload formats are listed below.
...
\midrule
\verb.[DEFINITIONS]. & \verb.<num_defs:int64>. then for each definition: \verb.<key_length:int32>. followed by \verb.<char[]>. \verb.<count:int32>. \verb.<type:char>. where type is \verb.c., \verb.i. or \verb.f., followed by \verb.<value[count]>. as \verb.char./\verb.int32./\verb.float64.. \\
\verb.[BLOCKS]. & \verb.<num_blocks:int64>. then per block: \verb.<duration_raster:int64>. \verb.<rf:int32>. \verb.<gx:int32>. \verb.<gy:int32>. \verb.<gz:int32>. \verb.<adc:int32>. \verb.<ext:int32>.. \\
\verb.[RF]. & \verb.<num:int64>. then per event: \verb.<id:int32>. \verb.<amp:float64>. \verb.<mag_id:int32>. \verb.<phase_id:int32>. \verb.<time_id:int32>. \verb.<center_ps:int64>. \verb.<delay_ps:int64>. \verb.<freq_ppm:float64>. \verb.<phase_ppm:float64>. \verb.<freq_off:float64>. \verb.<phase_off:float64>. \verb.<use:char>. \\
\verb.[GRADIENTS]. & \verb.<num:int64>. then per event: \verb.<id:int32>. \verb.<amp:float64>. \verb.<first:float64>. \verb.<last:float64>. \verb.<shape_id:int32>. \verb.<time_id:int32>. \verb.<delay_ps:int64>. \\
\verb.[TRAP]. & \verb.<num:int64>. then per event: \verb.<id:int32>. \verb.<amp:float64>. \verb.<rise_ps:int64>. \verb.<flat_ps:int64>. \verb.<fall_ps:int64>. \verb.<delay_ps:int64>. \\
\verb.[ADC]. & \verb.<num:int64>. then per event: \verb.<id:int32>. \verb.<num_samples:int64>. \verb.<dwell_ps:int64>. \verb.<delay_ps:int64>. \verb.<freq_ppm:float64>. \verb.<phase_ppm:float64>. \verb.<freq_off:float64>. \verb.<phase_off:float64>. \verb.<phase_shape_id:int32>. \\
\verb.[DELAYS]. (legacy) & \verb.<num:int64>. then repeated \verb.<id:int32>. \verb.<delay_ps:int64>.. Current readers ignore this section for compatibility with old files. \\
\verb.[SHAPES]. & \verb.<num_shapes:int64>. then per shape: \verb.<id:int32>. \verb.<num_uncompressed:int64>. \verb.<num_compressed:int64>. \verb.<compressed_data:float32[num_compressed]>. \\
\verb.[EXTENSIONS]. & \verb.<num:int64>. then per entry: \verb.<id:int32>. \verb.<type_id:int32>. \verb.<ref:int32>. \verb.<next:int32>.. \\
\bottomrule
\end{tabularx}
\end{center}
...
Section & Binary payload format \\
\midrule
\verb.extension TRIGGERS. payload & \verb.<ext_type_id:int32>. \verb.<num:int64>. then per trigger: \verb.<id:int32>. \verb.<type:int32>. \verb.<channel:int32>. \verb.<delay_ps:int64>. \verb.<duration_ps:int64>. \\
\verb.extension LABELSET. payload & \verb.<ext_type_id:int32>. \verb.<num:int64>. then per object: \verb.<id:int32>. \verb.<value:int32>. \verb.<label_index:int32>. \\
\verb.extension LABELINC. payload & \verb.<ext_type_id:int32>. \verb.<num:int64>. then per object: \verb.<id:int32>. \verb.<value:int32>. \verb.<label_index:int32>. \\
\verb.extension DELAYS. payload & \verb.<ext_type_id:int32>. \verb.<num:int64>. then per soft delay: \verb.<id:int32>. \verb.<num_id:int32>. \verb.<offset_ps:int64>. \verb.<factor:float64>. \verb.<hint_len:int32>. \verb.<hint:char[hint_len]>. \\
\verb.extension RF\_SHIMS. payload & \verb.<ext_type_id:int32>. \verb.<num:int64>. then per shim object: \verb.<id:int32>. \verb.<num_chan:int32>. \verb.<mag_phase_pairs:float64[2*num_chan]>. \\
\verb.extension ROTATIONS. payload & \verb.<ext_type_id:int32>. \verb.<num:int64>. then per rotation: \verb.<id:int32>. \verb.<q0:float64>. \verb.<qx:float64>. \verb.<qy:float64>. \verb.<qz:float64>. \\
\verb.[SIGNATURE]. binary payload & \verb.<type_len:int32>. \verb.<type:char[type_len]>. \verb.<hash_len:int32>. \verb.<hash:uint8[hash_len]>. \verb.<orig_size:int64>. where \verb.orig_size. is the original file length before appending the signature trailer. It is also the position in the final file, where the \verb.[SIGNATURE]. section starts.\\
\bottomrule
\end{tabularx}
\end{center}
...
\lstinputlisting{../tests/legacy/gredisp.seq}
