...
deltak=1/fov;
kRadius = round(Nx/2);
kSamples=round(2*pi*kRadius)*adcOversampling;
...
gradOversampling=true; % oversampling of the gradient shape, can be either true or false

% calculate a raw Archimedian spiral trajectory
clear ka;
ka(kRadius*kSamples+1)=1i; % init as complex
for c=0:kRadius*kSamples*tos_calculation
...
dt=sys.gradRasterTime/tos_calculation;
[ga, sa]=mr.traj2grad(ka,'RasterTime',dt,'firstGradStepHalfRaster',tos_calculation==1,'conservativeSlewEstimate',true);

% limit analysis
safety_magrin=0.99; % we need that  otherwise we just about violate the slew rate due to the rounding errors
dt_gabs=abs(ga(1,:)+1i*ga(2,:))/(sys.maxGrad*safety_magrin)*dt;
dt_sabs=sqrt(abs(sa(1,:)+1i*sa(2,:))/(sys.maxSlew*safety_magrin))*dt;

%figure;plot([dt_gabs; max(dt_gcomp); dt_sabs; max(dt_scomp)]');title('time stepping defined by gradient and slew-rate');
%%
dt_opt=max([dt_gabs;dt_sabs]);

% apply the lower limit not to lose the trajectory detail
dt_min=4*sys.gradRasterTime/kSamples/tos_calculation; % we want at least 4 points per revolution
dt_opt0=dt_opt;
...
kopt=interp1(t_smooth, ka', (0:floor(t_smooth(end)/dt_grad))*dt_grad)';
