function digest = md5(message, noBuiltIn)
    % digest = md5(message)
    %  Compute the MD5 digest of the message, as a hexadecimal digest.
    %  Per default uses built-in functions if available, but can be 
    %  forced to use the explicit source-code Matlab/Octave compatible 
    %  implementation by setting the optional parameter noBuiltIn to true.
    %  Note that the built-in functions are much faster than the 
    %  pure source-code implementation of the MD5 algorithm. 
    %  The latter is based on the source code from https://rosettacode.org/wiki/MD5
 
    if size(message,1)~=1 || ndims(message)~=2
        message=message(:).';
    end

    if ~exist('noBuiltIn','var') || ~noBuiltIn
        if mr.aux.isOctave()
            digest=hash('MD5',char(message)); % Octave-specific function
            return;
        else
            if ~isempty(message) % java function fails for the empty string so we revert to the source-code implementation
                digest=md5_java(message); % Matlab Java hack
                return;
            end
        end
    end
