...
% reads Siemens ASC ascii-formatted textfiles 
% return is a matlab structure with fields from the file(s).
% usage:
...
% starting from NumarisX va60a some files are split in two, like ####.asc
% and ####_GSWD_SAFETY.asc, therefore we need to attempt to read both parts
% and merge the structs
[FILEPATH,NAME,EXT]=fileparts(filePathBasic);
if exist([FILEPATH filesep NAME  '_GSWD_SAFETY' EXT],'file')
    filePaths={filePathBasic,[FILEPATH filesep NAME '_GSWD_SAFETY' EXT]};
else
    filePaths={filePathBasic};
end
