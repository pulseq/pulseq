...
int main(int argc, char** argv)
{
    ...
    for (iB = 0; iB < SequenceData.GetNumberOfBlocks(); ++iB)
    {
        ...
        if (pBlock->isSoftDelay())
        {
            ...
            if (nNumID >= 0 && nNumID < MAX_SOFT_DELAYS)
            {
                ...
                double dTest = double(-pBlock->GetSoftDelayEvent().offset) * pBlock->GetSoftDelayEvent().factor * 1e-3;
                if (pBlock->GetSoftDelayEvent().factor >= 0)
                {
        ...
        /*if (!pBlock->isSoftDelay()) // contributions of the soft delays to the measurement time are accounted for
                                    // differently, namely using m_vmapOffsetTimeFactor_Inner, m_vmapOffsetTimeFactor_...
        {
            switch (arbitrarySBB.getCurrentLabelValue(ONCE))
            {
                case 0:
    ...
    if (nRF) std::cout << "\t " << nRF << " RF pulses\n";
    if (nGradTrap) std::cout << "\t " << nGradTrap << " trapezoid gradients\n";
    if (nGradExtTrap) std::cout << "\t " << nGradExtTrap << " extended trapezoid gradients\n";
    if (nGradArb) std::cout << "\t " << nGradArb << " shaped gradients\n";
    if (nADC) std::cout << "\t " << nADC << " ADC events\n";
    if (nLBL) std::cout << "\t " << nLBL << " label extension events\n";
    if (nSoDe) std::cout << "\t " << nSoDe << " soft delay extension events\n";
